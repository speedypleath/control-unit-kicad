...
(478 violations before and after, byte-identical violation set). Their whole job
...
The 163-segment set that preceded this pipeline had **55 same-tier collinear
overlaps**. The current 203-segment set has **zero**.

## Pipeline

```bash
cd scripts
python3 gen_pin_map.py                     # -> pin_map.json  (GPIO <- nearest Teensy pin)
python3 gen_taps.py                        # -> taps_clean.json + net_meta.json
...
$KPY scripts/remove_wires.py <board.kicad_pcb>          # 239 -> 36 footprints
$KPY ~/KiCad/jumper-wires-kicad/scripts/place_wire.py \
     <board.kicad_pcb> scripts/segments_v3.json          # 36 -> 239
...
Colouring needs 6 tiers here; at whole-number steps the top wire would float 5mm
...
`taps_clean.json` (157 taps, 50 nets) and `net_meta.json` (per-net colour, label,
and endpoint `REF.PIN` pair) are generated by **`gen_taps.py`** from
`pin_map.json` — the GPIO assignment produced by `gen_pin_map.py`, which gives
every connector signal the Teensy pin physically nearest to it. That map is the
single source of truth shared by the schematic, the manufactured board's nets,
and this wiring; changing a pin means re-running `gen_pin_map.py` → `gen_taps.py`
→ `gen_segments_v3.py`, not editing segment JSON by hand.

`reconstruct_segments.py` recovers a segment list from wire footprints already on
a board (position, rotation, `m_Scale.x`, `m_Offset.z`, colour, label). It was how
the first clean tap set was salvaged and is still useful for inspecting a board
whose JSON has been lost — but it is **no longer the ground truth**. Deriving taps
from wire *geometry* is what produced the original bad `taps.json`, which counted
elbow corner points as taps; several of those happen to sit on an unrelated
component's pad, inventing connections that don't exist.
